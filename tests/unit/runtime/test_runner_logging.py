...
from nate_ntm.runtime.runner import _ExpectedAcpRequestFilter, _LOG_CONFIG
...
def _error_record(exc: BaseException) -> logging.LogRecord:
...
def test_expected_acp_request_filter_suppresses_replied_request_errors() -> None:
    filter_ = _ExpectedAcpRequestFilter()

    assert not filter_.filter(_error_record(RequestError.internal_error()))
    assert filter_.filter(_error_record(RuntimeError("real failure")))
