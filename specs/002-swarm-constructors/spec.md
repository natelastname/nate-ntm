...
**Feature Branch**: `[002-swarm-constructors]`
...
- How does swarm creation recover if a constructor performs an external side effect and a later constructor or final validation fails?
...
- **FR-017**: Constructors that perform external side effects MUST either be idempotent or provide compensating cleanup sufficient to prevent failed construction from leaving nate_ntm metadata in a valid-but-incomplete state.
