...
with an Agent Mail coordination service, plus two concrete
implementations:

* :class:`FakeAgentMailClient` – an in-memory, dev-mode implementation
  used in unit/integration tests that simulates projects, per-agent
  identities, and unread-mail flags without performing any network I/O.
* :class:`McpAgentMailClient` – a production-oriented adapter that talks
  to a running Agent Mail server (for example via the
  ``mcp_agent_mail`` package) over HTTP/JSON-RPC.
